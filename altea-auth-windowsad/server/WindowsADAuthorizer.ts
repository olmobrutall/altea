...
// The ONE authorizer of the three that can authenticate a typed password itself: `login` tries the local
// database FIRST (a DB round trip beats an LDAP bind), then a directory bind, then the database again so
// the ordinary failure messages come out of AuthLogic.
//
...
export class DirectoryServiceContext implements IAutoCreateUserContext {
    ...
    constructor(
        ...
        /** The sAMAccountName, which becomes the local `userName`. */
        readonly userName: string,
        /** What the directory was searched BY (a UPN or `DOMAIN\user`). */
...
export class WindowsADAuthorizer extends ADAuthorizer<WindowsADConfigurationEmbedded> implements IDirectoryInviter {
    ...
    /**
     * The local database first (a DB round trip beats an LDAP bind), then the directory,
    ...
    async loginWithWindowsADRegistry(userName: string, password: string): Promise<UserEntity | null> {
        return await AuthLogic.withDisabled(async () => {
            ...
            // Match by SID first, then fall back to the local name through AuthLogic.
    ...
    /** The user's transitive AD groups, by NAME and by objectGUID. */
