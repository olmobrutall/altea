...
describe("SqlFunctionsTest", { skip: !hasDb }, () => {
    ...
    // TODO(api): InSql() hint, and Start/End/Reverse/Replicate SQL string functions
    test("StringFunctions", async () => {
        ...
        await table(ArtistEntity).map(a => a.name.start(2)).toArray();
        await table(ArtistEntity).map(a => a.name.end(2)).toArray();
        await table(ArtistEntity).map(a => a.name.reverse()).toArray();
        await table(ArtistEntity).map(a => a.name.replicate(2)).toArray();
    });
