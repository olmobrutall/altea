...
// The DATA half of "a scheduled task can BE a process": widening `ScheduledTaskEntity.task` so it accepts a
// process algorithm.
//
// It lives HERE, not in the server bridge, because an implementedBy override changes what the SERIALIZER
// and the SCHEMA see — so both tiers must apply it, before anything is (de)serialized or the schema is
// built. An app calls it from its shared entity-overrides module; the server half (what actually happens
