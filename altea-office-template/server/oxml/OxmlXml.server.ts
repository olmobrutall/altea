// XML <-> OxmlElement tree. The read half delegates to `fast-xml-parser` (already a dependency of
// @altea/altea, used by the user-asset and AuthRules XML); the write half is hand-written so escaping,
// attribute order and empty-element form are exactly under our control — an OOXML part that round-trips
// with a subtly different encoding is a part Word may refuse to open.
//
// Only the XML subset OOXML actually uses is supported: one root element, an optional XML declaration and
// prolog, attributes, text, comments and CDATA. No DTDs and no external entities — which is also the right
// posture for documents uploaded by users.
//
...
import { OxmlCData, OxmlComment, OxmlElement, OxmlText, XmlTextWriter, type OxmlNode } from "./OxmlElement.server";
...
export class OxmlDocument {
    constructor(
        public root: OxmlElement,
        ...
        public readonly trailingNodes: OxmlNode[] = [],
    ) { }
}

export function parseXmlDocument(text: string): OxmlDocument {
    ...
    if (rootStart < 0)
        throw new Error("Malformed OOXML part: no root element");
    ...
    let root: OxmlElement | undefined;
    const trailingNodes: OxmlNode[] = [];
    for (const raw of nodes) {
        const converted = convertNode(raw);
        if (converted == null)
            continue;
        if (converted instanceof OxmlElement) {
            if (root != null)
                throw new Error("Malformed OOXML part: more than one root element");
            root = converted;
        } else if (root != null) {
    ...
    return new OxmlDocument(root, prolog, epilog, trailingNodes);
}
...
function convertNode(raw: Record<string, unknown>): OxmlNode | undefined {
    ...
    if (name === TEXT)
        return new OxmlText(decodeXmlEntities(String(value ?? "")));

    if (name === COMMENT)
        return new OxmlComment(rawInnerTextOf(value)); // comment bodies are not entity-encoded

    if (name === CDATA)
        return new OxmlCData(rawInnerTextOf(value));   // CDATA is literal by definition

    if (name.startsWith("?"))
        return undefined; // a processing instruction inside the body — OOXML does not use them

    const element = new OxmlElement(name);
...
export function serializeXmlDocument(doc: OxmlDocument): string {
...
export function serializeElement(element: OxmlElement): string {
    const writer = new XmlTextWriter();
