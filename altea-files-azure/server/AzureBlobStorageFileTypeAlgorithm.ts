...
// An IFileTypeAlgorithm whose store is an Azure Blob Storage container. Registered exactly like the
// local-folder one:
//
...
//   }));
//
// THREE things a REMOTE store cannot do, all enforced rather than approximated:
//  - **`renameAlgorithm` is REFUSED** (see the constructor): the collision probe is a network round-trip,
//    and the suffix is assigned in a SYNCHRONOUS hook before the INSERT, so a rename decided later could
//    not be written back to a row that already carries the old suffix. The default `calculateSuffix` puts
//    a GUID in the path, which is what makes one unnecessary.
//  - **`readAllBytesSync` THROWS**: there is no synchronous read of a remote blob. The one caller that
//    needs it is BigStringLogic (from the synchronous `retrieved` event), so a BigString column must not
//    be backed by this store.
//  - **the save is TWO-PHASE**: `prepareSuffix` (SYNC — validate, hash, assign the suffix, so the row can
...
/** How (and whether) a public URL to the blob is handed out. */
...
/** How long to wait for Microsoft Defender's malware verdict. */
export interface AzureDefenderPollingOptions {
    /** Total time to wait, in milliseconds (default 5 minutes). */
    totalWaitTime?: number;
    /** How often to re-check, in milliseconds (default 3 seconds). */
    pollInterval?: number;
}

/** Thrown when Defender's verdict says the uploaded file is malicious — or never arrives. */
...
export interface AzureBlobStorageFileTypeAlgorithmOptions extends FileTypeAlgorithmBaseOptions {
    /** The container this file's bytes live in (a multi-tenant app picks per file). */
    getClient: (fp: IFilePath) => ContainerClient;
    /** How a public URL is handed out (default None). */
    webDownload?: () => AzureWebDownload;
    /** The stored suffix (default Safe.yearMonth_Guid_Filename — its GUID is what makes the refused
     *  `renameAlgorithm` unnecessary). */
    calculateSuffix?: (fp: IFilePath) => string;
    /** The app does not own these blobs: never write, never delete. */
    weakFileReference?: boolean;
    /** Create the container on first write if it is missing. */
    createBlobContainerIfNotExists?: boolean;
    /** Download or Open (default Download, or Open when `onlyImages` is set). */
    getBlobAction?: (fp: IFilePath) => BlobAction;
    /** How long a SAS token stays valid, in milliseconds (default 15 minutes). */
    sasTokenExpires?: (fp: IFilePath) => number;
    /** The `Cache-Control` stamped on the blob. */
    getCacheControl?: (fp: IFilePath) => string | null;
    /** When set, an upload waits for Defender's malware verdict. */
...
export class AzureBlobStorageFileTypeAlgorithm extends FileTypeAlgorithmBase implements IFileTypeAlgorithm {
    ...
    /** The blob's metadata (length, content type, ETag, …). */
    ...
    /** The blob's bytes (a Node caller wants the bytes, not a stream). */
    ...
    /** The SYNC half: assign the suffix, so the row can be INSERTed with it. */
    ...
    /** The ASYNC half: create the container if asked, upload, then wait for Defender's verdict if that is
     *  configured. */
    ...
    /** Re-stamp content type / disposition / cache control on a stored blob. */
    ...
    /** A blob has no physical path. */
    fullPhysicalPath(_fp: IFilePath): string | undefined {
        return undefined;
    }

    /** The public URL, per `webDownload`. */
    fullWebPath(fp: IFilePath): string | undefined {
        ...
        const sas = generateBlobSASQueryParameters({
            ...
            // The resource is a blob rather than the container, which passing a blobName implies.
            blobName: suffix,
            permissions: BlobSASPermissions.from({ read: true }),
            // 5 minutes of backdating: it absorbs clock skew between us and the service.
    ...
    // The name of the last container created, so the CreateIfNotExists round-trip happens once per
    // container instead of once per file.
    ...
    private async ensureContainerExists(client: ContainerClient): Promise<void> {
        ...
        try {
            ...
            // ContainerAlreadyExists is swallowed: two hosts racing to create it is not an error.
    ...
    /** A Download is always `application/octet-stream` + `attachment`, so the browser saves it; an Open
     *  gets the file's real content type + `inline`, so the browser shows it. */
...
/** Kept even though the rename loop that used it is gone: it is the one question a caller may still want
 *  to ask, and a prefix listing is cheaper than a 404-per-probe. */
...
/**
 * Microsoft Defender for Storage scans an uploaded blob and writes its verdict as a blob TAG; this polls
 * for that tag and DELETES the blob if the verdict is bad — or never arrives. The point is that an upload
 * must not be reported as successful while a file Defender has not cleared is sitting in the container.
 */
async function checkForDefenderVerdict(
    ...
    try {
        ...
        // A verdict that never ARRIVES ends the same way as a bad one — the blob goes. Explicit here
