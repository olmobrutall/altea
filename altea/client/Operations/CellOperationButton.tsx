// Ported from Signum.React/Operations/CellOperationButton.tsx — copy-and-fix. altea fixes: import
// paths; dropped unused imports (Exception, MultiPropertySetterModal/PropertySetterComponentProps,
...
function getDefaultConfirmMessage(coc: CellOperationContext<Entity>) {
  ...
  if (coc.operationInfo.operationType == "Delete")
    return OperationMessage.PleaseConfirmYouWouldLikeToDelete0FromTheSystem.niceToString().formatHtml(
      <strong>{coc.lite.toString()} ({OperationMessage.As.niceToString()} {getTypeInfo(coc.lite.entityType).getNiceName()} {coc.lite.id})</strong>
    );
  else
