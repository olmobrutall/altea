// Ported from Signum.React/Operations/ContextualOperations.tsx — copy-and-fix. altea fixes: import
...
import { Operations, ContextualOperationContext } from '../Operations'
import type { ContextualOperationSettings, EntityOperationSettings } from '../Operations'
...
export namespace ContextualOperations {
    ...
    export async function getOperationsContextualItems(ctx: ContextualItemsContext<Entity>): Promise<MenuItemBlock | undefined> {
        ...
        if (contexts.some(coc => coc.operationInfo.hasCanExecute || coc.operationInfo.hasCanExecuteExpression || coc.operationInfo.hasStates) || Operations.Options.maybeReadonly(ti)) {
            ...
            if (Navigator.isReadOnly(ti.ctor! as any, { ignoreTypeIsReadonly: true })) {
    ...
    function getDefaultConfirmMessage(coc: ContextualOperationContext<Entity>) {
        ...
        if (coc.context.lites.length > 1) {
            var message = coc.context.lites
                ...
                .map(gr => gr.elements.length + " " + (gr.elements.length == 1 ? getTypeInfo(gr.key).getNiceName() : getTypeInfo(gr.key).getNicePluralName()))
          ...
          if (coc.operationInfo.operationType == "Delete")
            return OperationMessage.PleaseConfirmYouWouldLikeToDelete0FromTheSystem.niceToString().formatHtml(<strong> {lite.toString()} ({OperationMessage.As.niceToString()} {getTypeInfo(lite.entityType).getNiceName()} {lite.id})</strong>);
            else
            return OperationMessage.PleaseConfirmYouWouldLikeTo01.niceToString().formatHtml(<strong>{coc.operationInfo.niceName}</strong>, <strong>{lite.toString()} ({OperationMessage.As.niceToString()} {getTypeInfo(lite.entityType).getNiceName()} {lite.id})</strong>);
    ...
    export function OperationMenuItem({ coc, onOperationClick, onClick, extraButtons, color, icon, iconColor, children }: OperationMenuItemProps): React.ReactElement {
    ...
    export namespace OperationMenuItem {
        export function getText(coc: ContextualOperationContext<any>): React.ReactNode {
        ...
        export function simplifyName(niceName: string): string {
                    ...
                    return Operations.API.constructFromMany(lites, coc.operationInfo.key, ...args)
                        .then(coc.onConstructFromSuccess ?? (pack => {
                            ...
                            Navigator.createNavigateOrTab(pack, coc.event!)
                                .then(() => coc.context.markRows(lites.toObject(l => l.key(), lite => null)));
                        }));
                }
