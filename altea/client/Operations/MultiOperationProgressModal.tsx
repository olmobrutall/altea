// Ported from Signum.React/Operations/MultiOperationProgressModal.tsx — copy-and-fix. altea fixes:
// import paths (Globals→entities/globals, Signum.Entities→entities/*, Signum.External/Operations→
// entities/*); `.EntityType`→`.entityType`, `liteKey(x)`→`x.key()`; dropped unused named `useState`;
// `import type { Operations }` (type-only, breaks the cycle).
...
export function MultiOperationProgressModal(p: MultiOperationProgressModalProps): React.ReactElement {
  ...
  var typeNiceName = React.useMemo(() =>
    p.lites.map(a => a.entityType).distinctBy(a => a).map(a => p.lites.length == 1 ? getTypeInfo(a).getNiceName() : getTypeInfo(a).getNicePluralName()).joinComma(CollectionMessage.And.niceToString()), [p.lites]);
  ...
  function handleOnExited() {
    p.onExited!({ errors: operationResultsRef.current.toObject(a => a.entity.key(), a => a.error ?? null) });
  }
