// Ported from Signum.React/Operations/EntityOperations.tsx — copy-and-fix. altea fixes: import paths;
...
import type { EntityOperationSettings, EntityOperationGroup, AlternativeOperationSetting } from '../Operations'
import { ServiceError } from '../Services'
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import type { IconProp } from "@fortawesome/fontawesome-svg-core";
import { Dropdown, DropdownButton, Button, OverlayTrigger, Tooltip, ButtonGroup } from "react-bootstrap";
...
export namespace EntityOperations {
  ...
  export function getEntityOperationButtons(ctx: ButtonsContext): Array<ButtonBarElement | undefined> | undefined {
    const ti = tryGetTypeInfo(ctx.pack.entity);
  ...
  export function andNew<T extends Entity>(eoc: EntityOperationContext<T>, inDropdown?: boolean, reloadComponent: boolean = true): AlternativeOperationSetting<T> {
    ...
    return ({
      ...
      isVisible: eoc.frame!.allowExchangeEntity && eoc.frame.createNew != null && Navigator.isCreable(getTypeName(eoc.entity), { customComponent: true, isSearch: true }),
      inDropdown: inDropdown,
      onClick: () => {
        eoc.onExecuteSuccess = pack => {
          Operations.notifySuccess();
  ...
  export function defaultExecuteLite<T extends Entity>(eoc: EntityOperationContext<T>, ...args: any[]): Promise<void | undefined> {
    ...
    return confirmInNecessary(eoc).then(conf => {
      ...
      var promise = eoc.progressModalOptions ?
        Operations.API.executeLiteWithProgress(eoc.entity.toLite(), eoc.operationInfo.key, eoc.progressModalOptions, ...args) :
        Operations.API.executeLite(eoc.entity.toLite(), eoc.operationInfo.key, ...args);
  ...
  function getDefaultConfirmMessage<T extends Entity>(eoc: EntityOperationContext<T>) {
    ...
    if (eoc.operationInfo.operationType == "Delete")
      return OperationMessage.PleaseConfirmYouWouldLikeToDelete0FromTheSystem.niceToString().formatHtml(
        <strong>{eoc.entity.toString()} ({OperationMessage.As.niceToString()} {getTypeInfo(eoc.entity).getNiceName()} {eoc.entity.id})</strong>
      );
    else
