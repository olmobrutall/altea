// Ported from Signum.React/Frames/FrameModal.tsx — copy-and-fix (faithful, replaces the earlier
// reconstruction). altea fixes: import paths (ViewPromise←EntitySettings, ReadonlyBinding←entities/
// binding, entityInfo/GraphExplorer←Reflection); ModifiableEntity→BaseEntity; the local getTypeName
// (Signum read `.EntityType`/`.Type`) delegates to Reflection.getTypeName + isEntityPack; `.Type`→
// getTypeName/tryGetTypeInfo(entity); entity `.modified`→isGraphModified; `.isNew` cast to Entity;
// isTypeModel(name) has no altea equivalent → `validate` defaults false (validation is save-path).
import * as React from 'react'
import { openModal, FunctionalAdapter } from '../Modals'
import type { IModalProps, IHandleKeyboard, IGetUIState, UIState } from '../Modals'
...
import type { StyleOptions, EntityFrame, IHasChanges, ButtonsContext } from '../TypeContext'
import { Entity, BaseEntity, EmbeddedEntity } from '../../data/entity'
...
interface FrameModalProps<T extends BaseEntity> extends IModalProps<T | undefined> {
...
interface FrameModalState<T extends BaseEntity> {
  pack: EntityPack<T>;
  lastEntity: string;
  refreshCount: number;
...
export function FrameModal<T extends BaseEntity>(p: FrameModalProps<T>): React.JSX.Element {
  ...
  // Names the dialog through aria-labelledby on the <Modal> below. Unique per instance because search,
  // selector and frame dialogs are opened nested here, so a fixed id would be ambiguous.
  ...
  function getSaveChangesOperations() {
    ...
    const ti = tryGetTypeInfo(frame.pack.entity)

    const pack = frame.pack;

    const buttonContext: ButtonsContext = {
      frame: frame as unknown as EntityFrame<BaseEntity>,
      pack: pack,
  ...
  function handleOkClicked(): void {
    ...
    if (hasChanges() &&
      (p.requiresSaveOperation != undefined ? p.requiresSaveOperation : Navigator.typeRequiresSaveOperation(getTypeName(pack!.entity)))) {
      MessageModal.show({
      ...
      Navigator.API.validateEntity(pack!.entity as unknown as Entity)
        .then(() => {
          okClicked.current = true;
          setShow(false);
  ...
  function handleOnExited() {
    if (okClicked.current)
      ...
      if (p.buttons == "close") { //Even if you cancel, maybe you have executed an operation
        p.onExited!(isGraphModified(state.pack.entity) ? undefined : state.pack.entity);
      }
      else {
        p.onExited!(undefined);
      }
  ...
  var settings = state && Navigator.getSettings(getTypeName(state.pack.entity));

  let frame: EntityFrame<T>;
  let wc: WidgetContext<BaseEntity> | undefined = undefined;
  ...
  if (state) {
    ...
    styleOptions = {
      readOnly: p.readOnly != undefined ? p.readOnly : Navigator.isReadOnly(state.pack, { isEmbedded: p.propertyRoute?.type.is(EmbeddedEntity) }),
      frame: frame as unknown as EntityFrame<BaseEntity>,
    };

    ctx = new TypeContext(undefined, styleOptions, pr, new ReadonlyBinding(state.pack.entity, ""), prefix!);

    wc = { ctx: ctx, frame: frame as unknown as EntityFrame<BaseEntity> };
  }
      ...
      <div className="modal-body" style={pc.executing == true ? { opacity: ".6" } : undefined}>
        {wc && <WidgetEmbedded widgetContext={wc} >
          <div className="sf-button-widget-container">
            {entityComponent.current && <ButtonBar ref={buttonBar} frame={frame as unknown as EntityFrame<BaseEntity>} pack={pc.pack} isOperationVisible={p.isOperationVisible as any} />}
          </div>
...
export namespace FrameModalManager {
  export function openView<T extends BaseEntity>(entityOrPack: Lite<T & Entity> | T | EntityPack<T>, options: Navigator.ViewOptions<T>): Promise<T | undefined> {
    ...
    return openModal<T>(<FrameModal
      ...
      isOperationVisible={options.isOperationVisible as any}
      ...
      buttons={options.buttons ?? Navigator.typeDefaultButtons(getTypeName(entityOrPack), options.propertyRoute?.type.is(EmbeddedEntity))} />);
  }
}
...
export function FrameModalTitle({ pack, pr, title, subTitle, widgets, getViewPromise }: {
  ...
  function renderExpandLink(entity: BaseEntity) {
    ...
    if (ti == undefined || !Navigator.isViewable(ti.ctor! as any)) //Embedded
      return undefined;
