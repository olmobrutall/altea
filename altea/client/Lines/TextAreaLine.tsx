...
export const TextAreaLine: (props: TextAreaLineProps) => React.ReactNode | null
  ...
  // valueHtmlAttributes last, matching the editable path below: what the caller passes explicitly has to
  // beat the generated defaults, or a line cannot correct the name of the control it wraps.
      ...
      <FormGroup ctx={p.ctx} error={p.error} label={p.label} labelIcon={p.labelIcon} helpText={helpText} helpTextOnTop={helpTextOnTop} htmlAttributes={{ ...c.baseHtmlAttributes(), ...p.formGroupHtmlAttributes }} labelHtmlAttributes={p.labelHtmlAttributes} ariaAttributes={ariaAtts}>
        {inputId => <>
          ...
          {/* readOnly, not disabled. A disabled control is out of the tab order and its value is not
              announced, so every read-only multi-line field in the application was simply missing for a
              screen reader user. readOnly keeps it uneditable while leaving it focusable and readable,
              and the grey background is the same either way. */}
  ...
  // TODO(port): Signum uses EntityControlMessage (localized, gender/number-aware) here; inlined English.
  var cc = c.props.charCounter == null ? null :
    c.props.charCounter == true && c.props.valueHtmlAttributes?.maxLength ? <ChartCounter myRef={ccRef}>{v => {
      ...
      return <><strong className={rem <= 0 ? "text-danger" : undefined}>{rem}</strong> characters remaining</>;
    }}</ChartCounter> :

      c.props.charCounter == true ? <ChartCounter myRef={ccRef}>{v => <><strong>{v}</strong> characters</>}</ChartCounter> :
...
export function taskSetHtmlProperties(lineBase: LineBaseController<LineBaseProps, unknown>, state: LineBaseProps): void {
  ...
  if (vl && pr?.propertyRouteType == PropertyRouteType.FieldOrProperty) {

    var member = pr.fieldInfo!;
