...
export function FormGroup(p: FormGroupProps): React.ReactElement {
  ...
  // exist, so clicking it focused nothing — a date, an entity picker and a markdown line each — and the
  // association it claimed was not there. Those controls carry their own aria-label, so the NAME survives;
  // what is restored here is the click. The attribute is dropped rather than left dangling, so the markup
  ...
  function focusControl(e: React.MouseEvent<HTMLLabelElement>): void {
    if (hasControl)
      return;
