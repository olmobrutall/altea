 ...
 * conveyed at all. The call site cannot override it, because `DropdownItem` renders
 * `<Component {...props} {...dropdownItemProps} />` and the library's props come last. Leaving `active`
 * out is what drops the attribute — with no `eventKey` either, `useDropdownItem` resolves it to undefined
 * and React omits it — so the class and the state are stated here instead.
 *
 ...
 *
 * Ported from Signum.React/Components/DropdownActive.tsx.
 */
