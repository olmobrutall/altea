// Ported from Signum.React/SearchControl/SearchValue.tsx — copy-paste + fix. altea fixes: imports
// retargeted (Signum.Entities→entities/*, QueryValueRequest→entities/dynamicQuery/queryRequest,
// Globals→entities/globals, Lines barrel→PropertyRoute+StyleContext); dropped the UNUSED luxon +
// Reflection format helpers (DateTime/Duration/toLuxonFormat/toLuxonDurationFormat/timeToString/
// toFormatWithFixes/getEnumInfo/toNumberFormat — value formatting is delegated to Finder cell
...
function SearchValue(p: SearchValueProps): React.ReactNode | null {
    ...
    (p.findOptions instanceof Lite) ? { queryName: p.findOptions.entityType, filterOptions: [{ token: "Entity", value: p.findOptions }] } :
      (p.findOptions instanceof Entity) ? { queryName: getTypeName(p.findOptions), filterOptions: [{ token: "Entity", value: p.findOptions }] } :
        p.findOptions;
    ...
    function isMultiLine() {
      let token = valueToken;
      return token && token.filterType == "String" && token.getPropertyRoute()?.fieldInfo?.isMultiline;
    }
    ...
    function handleClick(e: React.MouseEvent<any>) {
      ...
      if (findOptions.columnOptions == undefined && valueToken && valueToken.parent)
        fo = {
          ...findOptions,
          columnOptions: [{
            token: valueToken.isAggregate() ? valueToken.parent!.fullKey() : valueToken.fullKey(),
            summaryToken: valueToken.isAggregate() ? valueToken.fullKey() : undefined,
          }],
