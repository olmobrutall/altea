// Ported from Signum.React/SearchControl/FilterBuilder.tsx — copy-paste + fix. altea fixes:
//   - QueryToken is a CLASS: `token.niceName`→`niceName()`, `token.type.name`→`type.getTypeName()` /
...
//     companion VALUES from entities/dynamicQueries (values()/niceToString()); Binding from ../binding;
...
export default function FilterBuilder(p: FilterBuilderProps): React.ReactElement {
  ...
  function handlerNewFilter(e: React.MouseEvent<any>, isGroup: boolean) {
    ...
    p.filterOptions.push(isGroup ?
      {
        groupOperation: "Or",
        token: lastToken && lastToken.hasAnyOrAll() ? lastToken.getTokenParents().filter(a => a.isAnyOrAll()).lastOrNull() : undefined,
        filters: [],
        frozen: false,
        expanded: true,
  ...
  function handleDrop(e: React.DragEvent<HTMLTableRowElement>) {
    ...
    if (dropInfo.mode == "InsideFirst" || dropInfo.mode == "InsideLast") {
      ...
      if (isFilterCondition(draggingFilter) && dropInfo.targetFilter.token == null && draggingFilter.token && draggingFilter.token.hasAnyOrAll()) {
        const anyOrAllToken = draggingFilter.token.getTokenParents().filter(a => a.isAnyOrAll()).lastOrNull();
        if (anyOrAllToken) {
...
export function FilterGroupComponent(p: FilterGroupComponentsProps): React.ReactElement | null {
  ...
  function handlerNewFilter(e: React.MouseEvent<any>, isGroup: boolean) {
    ...
    if (!lastToken || p.filterGroup.token && !p.filterGroup.token.isPrefixOf(lastToken))
      lastToken = p.filterGroup.token;

    p.filterGroup.filters.push(isGroup ?
      {
        groupOperation: p.filterGroup.groupOperation == "And" ? "Or" : "And",
        token: lastToken && lastToken.hasAnyOrAll() ? lastToken.getTokenParents().filter(a => a.isAnyOrAll()).lastOrNull() : p.prefixToken,
        filters: [],
        frozen: false,
        expanded: true,
      } as FilterGroupOptionParsed :
  ...
  function handleTokenChanged(newToken: QueryToken | null | undefined) {
    ...
    if (newToken == undefined) {
      ...
      if (!areEqual(f.token, newToken, a => a.filterType) ||
        ...
        newToken.filterType == "Lite" && f.value != null && !newToken.type.isByAll() && !newToken.type.typeInfos().some(t => t.ctor === (f.value as Lite<any>).entityType)) {
