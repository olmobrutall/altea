...
//   - MANUAL sub-tokens resolve to REAL ManualToken instances (altea has no `queryTokenType` string
//     discriminator, so there is nothing to synthesize a flat DTO from). The `[Operations]` container
//     does NOT come through here at all: its leaves are ordinary metadata sub-tokens, generated by the
//     token itself (data/dynamicQuery/tokens/operationToken), so it takes the TokenCompleter path below.
