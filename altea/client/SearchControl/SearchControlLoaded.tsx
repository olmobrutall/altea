// Ported from Signum.React/SearchControl/SearchControlLoaded.tsx — copy-paste + fix. altea fixes:
//   - imports retargeted (Signum.Entities→entities/*, ResultTable/ResultRow/QueryRequest/Pagination→
//     entities/dynamicQuery/queryRequest, DynamicQuery/External→entities/dynamicQueries, Globals→
//     entities/globals + domGlobals, Signum.Basics→entities/typeEntity + Basics/VisualTipIcon stub).
//   - luxon DateTime → Temporal; csstype dropped. QueryToken is a class (`.fullKey()`→`fullKey()`,
//     `.niceName()`→`niceName()`, `.type.name`→`.type.getTypeName()`, `.type.isCollection`→`.type.array`);
//     `queryTokenType == "X"` → is*(token) predicates. Removed Reflection fns swept: `getTypeInfos(x)`/
//     `tryGetTypeInfos(x)`→`x.typeInfos()`, `isTypeModel(e)`→`e instanceof ModelEntity`. Signum.Entities
//     idioms: `is(a,b)`→`a.is(b)`, `getToString`→`.toString()`, `isEntity/isLite`→instanceof.
...
import { renderContextualItems, type ContextualItemsContext, type ContextualMenuItem, type MarkedRowsDictionary, type MarkedRow, type SearchableMenuItem, type ContextMenuPack } from './ContextualItems'
...
export class SearchControlLoaded extends React.Component<SearchControlLoadedProps, SearchControlLoadedState> {
  ...
  entityColumnTypeInfos(): TypeInfo[] {
    return this.entityColumn().type.typeInfos();
  }
          ...
          Finder.getPropsFromFilters(tn, this.props.findOptions.filterOptions)
            ...
            .then(pack => {
              if (pack) {
                ...
                // TODO(port): Signum's Navigator.createInCurrentTab / createInNewTab (new-tab open vs
                // in-place SPA nav for a freshly-constructed entity) → altea routes both through the
                // Navigator.view stub for now (Frames not ported).
                Navigator.view(pack, { getViewPromise: viewName ? () => viewName : undefined });
              }
            });
        } else {
  ...
  renderContextualMenu(): React.ReactElement | null {
    ...
    function isColumnGroupable(columnIndex: number) {
      var token = fo.columnOptions[columnIndex].token;
      return token && !token.hasOperation() && !token.hasToArray();
    }
  ...
  renderHeaders(): React.ReactNode {
    ...
    var rootKeys = !this.props.findOptions.groupResults ? [] : getRootKeyColumn(this.props.findOptions.columnOptions.filter(co => co.token && !co.token.isAggregate() && !co.token.hasToArray()));
        ...
        {visibleColumns.map(({ column: co, cellFormatter, columnIndex: i }) =>
          ...
          <th key={i}
            ...
            onKeyDown={e => {
              ...
              // right-click opens, and a right-click was the only way to it. The context menu key and
              // Shift+F10 are that same gesture on a keyboard. Handled here rather than left to the
              ...
              // is what stops it opening a second time.
  ...
  canOrder(column: ColumnOptionParsed): boolean {
    ...
    if (t.type.is(EmbeddedEntity) || t.type.is(ModelEntity) || t.type.getTypeName() == "CellOperationDTO" || t.type.getTypeName() == "ManualCellDTO")
      return false; // TODO(port): Signum's QueryToken.hasOrderAdapter — altea has no order-adapter flag, so embedded/model tokens aren't orderable yet.
  ...
  static getGroupFilters(row: ResultRow, resTable: ResultTable, resFo: FindOptionsParsed): FilterOption[] {
    ...
    var rootKeys = getRootKeyColumn(resFo.columnOptions.filter(co => co.token && !co.token.isAggregate() && !co.token.hasOperation() && !co.token.hasManual() && co.token.fullKey() != QueryTokenString.timeSeries.token));
    ...
    var extraColumns = resFo.columnOptions.map(a =>
      a.token == null ? null :
        a.token.fullKey() == QueryTokenString.timeSeries.token ? null :
          a.token.isAggregate() ? (!a.token.parent ? null : ({ token: a.token.parent.fullKey(), summaryToken: a.token.fullKey() }) as ColumnOption) :
  ...
  renderRows(): React.ReactNode {
    ...
    const canOpenRow = (row: ResultRow): boolean =>
      !(!row.entity || Navigator.entitySettings()[cleanTypeName(row.entity.entityType)]?.isViewableLite?.(row.entity, { isSearch: "main" }) === false);
    ...
    return resultTable.rows.map((row, i, rows) => {
      ...
      function equals(a: unknown, b: unknown) {
        return a === b || (a != null && (a as any).is(b as any));
      }
      ...
      var tr = (
        ...
        <tr
          ...
          // mouse-only. The entity column's own link still opens the same row, so this adds a SECOND way
          // to what it does rather than the first, but it is what makes the table navigable.
  ...
  renderRowsMobile(): React.ReactElement | React.ReactElement[] {
    ...
    return resultTable.rows.map((row, i) => {
      ...
      var div = (
        <div key={i} data-row-index={i} data-entity={row.entity && row.entity.key()}
          onDoubleClick={e => this.handleDoubleClick(e, row, resultTable.columns)}
          {...ra}
  ...
  getMarkedRow(row: ResultRow): MarkedRow | undefined {
    ...
    var key = this.props.querySettings?.markRowsColumn ? row.columns[this.state.resultTable!.columns.indexOf(this.props.querySettings?.markRowsColumn)] : row.entity && row.entity.key();
...
function dominates(root: QueryToken, big: QueryToken) {
  ...
  if (big.hasElement()) {
    ...
    var elemBig = big.getTokenParents().last(a => a.isElement());
    var elemRoot = root.getTokenParents().last(a => a.isElement());
...
function niceCount(count: number, ti: TypeInfo) {
  return <span><strong>{count}</strong> {count == 1 ? ti.getNiceName() : ti.getNicePluralName()}</span>;
}
