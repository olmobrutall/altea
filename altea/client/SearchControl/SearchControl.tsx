  ...
  return (
    <ErrorBoundary>
      ...
      <SearchControlLoaded ref={searchControlLoaded}
        ...
        showSystemTimeButton={SearchControlOptions.showSystemTimeButton(handler, p) && (qs?.allowSystemTime ?? tis.some(a => a.systemVersioned != null))}
        ...
        create={p.create != null ? p.create : (qs?.allowCreate ?? true) && (fop => tis.some(ti => Navigator.isCreable(cleanTypeName(ti.ctor!), { isSearch: true })))}
        createButtonClass={p.createButtonClass}

        view={p.view != null ? p.view : tis.some(ti => Navigator.isViewable(cleanTypeName(ti.ctor!), { isSearch: "main" }))}
