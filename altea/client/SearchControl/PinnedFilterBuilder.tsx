// Ported from Signum.React/SearchControl/PinnedFilterBuilder.tsx — copy-paste + fix. altea fixes:
//   - Binding from ../binding; SearchMessage from ../../entities/uiMessages; classes from
//     ../../entities/globals.
//   - QueryToken is a class: `token.niceName`→`token.niceName()`; the `queryTokenType == "AnyOrAll" |
//     "Element"` discriminator → `token.isAnyOrAll() | token.isElement()` (altea has no discriminator).
...
export default function PinnedFilterBuilder(p: PinnedFilterBuilderProps): React.ReactElement | null {
  ...
  function renderValue(filter: FilterOptionParsed, isFirst: boolean) {
    ...
    var label = f.pinned!.label || (f.token?.isAnyOrAll() || f.token?.isElement() ? f.token!.parent?.niceName() : f.token?.niceName());
