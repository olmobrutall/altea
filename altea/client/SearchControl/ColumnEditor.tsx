// Ported from Signum.React/SearchControl/ColumnEditor.tsx — copy-paste + fix. altea fixes:
//   - messages from entities/uiMessages; CombineRows (value/niceToString companion) from
//     entities/dynamicQueries; StyleContext from ../TypeContext.
//   - QueryToken is a class: `.niceName`→`niceName()`, `token.type.isCollection`→`token.type.array`,
//     `summaryToken.queryTokenType != "Aggregate"`→`!....isAggregate()`; the exotic Operation/Manual/
...
export function columnError(token?: QueryToken | undefined): string | undefined {
  ...
  // TODO(port): the exotic OperationContainer / Manual / IndexerContainer token kinds aren't modelled
  // in altea yet, so there's no "invalid column expression" case to detect here.
