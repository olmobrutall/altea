// Ported from Signum.React/SearchControl/SearchValueLine.tsx — copy-paste + fix. altea fixes: imports
// retargeted (Signum.Entities→entities/*, QueryValueRequest→queryRequest, ViewPromise→EntitySettings);
// idioms isEntity/isLite→instanceof; `getTypeInfos(x)`/`tryGetTypeInfos(x)`→`x.typeInfos()`.
...
import { getQueryKey, getQueryNiceName, getTypeName } from '../Reflection'
...
import SearchValue, { renderTimeMachineIcon, type SearchValueController } from './SearchValue'
...
export default function SearchValueLine(p: SearchValueLineProps): React.JSX.Element | null {
  ...
  function getFindOptions(props: SearchValueLineProps): FindOptions {
    if (props.findOptions) {
      ...
      if ((fo instanceof Entity))
        return {
          queryName: getTypeName(fo),
          filterOptions: [{ token: QueryTokenString.entity(), value: fo }]
        };

      if ((fo instanceof Lite))
        return {
    ...
    if ((ctx.value instanceof Entity))
      return {
        queryName: getTypeName(ctx.value),
        filterOptions: [{ token: QueryTokenString.entity(), value: ctx.value }]
      };

    if ((ctx.value instanceof Lite))
      return {
  ...
  const isQuery = p.valueToken == undefined || token?.isAggregate();
  ...
  const view = value != undefined && (p.viewEntityButton ?? ((value instanceof Lite) && Navigator.isViewable(getTypeName(value)))) &&
