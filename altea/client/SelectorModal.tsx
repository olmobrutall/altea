// Ported from Signum.React/SelectorModal.tsx — copy-paste + fix. altea fixes: imports retargeted
// (Modals/Reflection/Components siblings; messages→entities/uiMessages; Lite/Entity→entities/*);
// idioms getToString(x)→x.toString(), liteKey(x)→x.key(), TypeInfo.niceName→getNiceName(),
...
namespace SelectorModal {
  ...
  export function chooseType(options: TypeInfo[], config?: SelectorConfig<TypeInfo>): Promise<TypeInfo | undefined> {
    return SelectorModal.chooseElement(options,
      {
        buttonDisplay: a => a.getNiceName(),
        buttonName: a => cleanTypeName(a.ctor!),
  ...
  export function chooseLite<T extends Entity>(type: Type<T> | TypeInfo | string, values: Lite<T>[], config?: SelectorConfig<Lite<T>>): Promise<Lite<T> | undefined> {
    const ti = type instanceof TypeInfo ? type : getTypeInfo(type);
    return SelectorModal.chooseElement<Lite<T>>(values.orderBy(l => l.toString().toLowerCase()),
      {
        buttonDisplay: a => a.toString(),
        buttonName: a => a.key(),
        title: SelectorMessage._0Selector.niceToString(ti.getNiceName()),
        message: SelectorMessage.PleaseChooseA0ToContinue.niceToString(ti.getNiceName()),
        size: "md",
        ...config
      });
  }
