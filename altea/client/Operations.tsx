// Ported from Signum.React/Operations.tsx — copy-and-fix (faithful). Replaces the earlier simplified
// reconstruction. altea fixes: import paths retargeted; Signum's ConstructSymbol_Simple/_From/_FromMany
// → altea ConstructSymbol<T, Simple|From<F>|FromMany<F>>; getToString(x)→x.toString(), toLite(x)→
// x.toLite(), lite.EntityType→lite.entityType; GraphExplorer.propagateAll is altea's no-op shim.
// DEFERRED: the operation-log quick link in start() needs the QuickLink subsystem + OperationLogEntity
// (a DB entity) — commented TODO(port). Everything else is faithful.
...
import type { TypeInfo } from '../entities/reflection';
...
import { EngineMessage, JavascriptMessage, OperationMessage } from '../entities/uiMessages';
import type { ConstructSymbol, From, FromMany, Simple, DeleteSymbol, ExecuteSymbol, OperationSymbol, PropertyOperation } from '../entities/operations';
...
export namespace Operations {
  ...
  export namespace API {
    ...
    export function construct<T extends Entity>(type: string | Type<T>, operationKey: string | ConstructSymbol<T, Simple>, ...args: any[]): Promise<EntityPack<T> | undefined> {
      return ajaxPost({ url: "/api/operation/construct/" + getOperationKey(operationKey) }, { args, type: getTypeName(type) });
    }

    export function constructFromEntity<T extends Entity, F extends Entity>(entity: F, operationKey: string | ConstructSymbol<T, From<F>>, ...args: any[]): Promise<EntityPack<T> | undefined> {
    ...
    export function constructFromLite<T extends Entity, F extends Entity>(lite: Lite<F>, operationKey: string | ConstructSymbol<T, From<F>>, ...args: any[]): Promise<EntityPack<T> | undefined> {
    ...
    export function constructFromMultiple<T extends Entity, F extends Entity>(lites: Lite<F>[], operationKey: string | ConstructSymbol<T, From<F>>, options: MultiOperationOptions, ...args: any[]): Promise<ErrorReport> {
    ...
    export function constructFromMany<T extends Entity, F extends Entity>(lites: Lite<F>[], operationKey: string | ConstructSymbol<T, FromMany<F>>, ...args: any[]): Promise<EntityPack<T> | undefined> {
      GraphExplorer.propagateAll(lites, args);
    ...
    export function executeLiteWithProgress<T extends Entity>(lite: Lite<T>, operationKey: string | ExecuteSymbol<T>, options: OperationWithProgressOptions, ...args: any[]): Promise<EntityPack<T>> {
      ...
      var modalOptions: ProgressModalOptions = {
        ...
        message: options.message ?? lite.toString(),
...
export class ConstructorOperationSettings<T extends Entity> extends OperationSettings {
  ...
  constructor(operationSymbol: ConstructSymbol<T, Simple> | string, options: ConstructorOperationOptions<T>) {
...
export class ConstructorOperationContext<T extends Entity> {
  ...
  defaultConstruct(...args: any[]): Promise<EntityPack<T> | undefined> {
    return Operations.API.construct<T>(getTypeName(this.typeInfo.ctor! as any), this.operationInfo.key, ...args);
  }
...
export class ContextualOperationSettings<T extends Entity> extends OperationSettings {
  ...
  constructor(operationSymbol: ConstructSymbol<any, FromMany<T>> | string, options: ContextualOperationOptions<T>) {
    super(operationSymbol);
...
export class EntityOperationContext<T extends Entity> {
  ...
  static fromChildTypeContext<T extends Entity>(ctx: TypeContext<T>, operation: ExecuteSymbol<T> | DeleteSymbol<T> | ConstructSymbol<any, From<T>> | string, canExecute: string | undefined): EntityOperationContext<T> | undefined {
    ...
    result.canExecute = canExecute ?? (ctx.value.isNew && !oi.canBeNew ? EngineMessage.TheEntity0IsNew.niceToString(ctx.value.toString()) : undefined);
    result.complete();
    return result;
  }

  static fromTypeContext<T extends Entity>(ctx: TypeContext<T>, operation: ExecuteSymbol<T> | DeleteSymbol<T> | ConstructSymbol<any, From<T>> | string): EntityOperationContext<T> | undefined {
    if (!ctx.frame)
  ...
  static fromEntityPack<T extends Entity>(frame: EntityFrame<any>, pack: EntityPack<T>, operation: ExecuteSymbol<T> | DeleteSymbol<T> | ConstructSymbol<any, From<T>> | string): EntityOperationContext<T> | undefined {
    ...
    result.settings = Operations.getSettings(operationKey) as EntityOperationSettings<T>;
    result.canExecute = (pack?.canExecute && pack.canExecute[operationKey]) ?? (pack.entity.isNew && !oi.canBeNew ? EngineMessage.TheEntity0IsNew.niceToString(pack.entity.toString()) : undefined);
    result.complete();
    return result;
  }
  ...
  onDeleteSuccess_Default = (): void => {
    this.frame.onClose();
    Navigator.raiseEntityChanged(this.entity);
    Operations.notifySuccess();
  }
...
export class EntityOperationSettings<T extends Entity> extends OperationSettings {
  ...
  constructor(operationSymbol: ExecuteSymbol<T> | DeleteSymbol<T> | ConstructSymbol<any, From<T>> | string, options: EntityOperationOptions<T>) {
    super(operationSymbol)
