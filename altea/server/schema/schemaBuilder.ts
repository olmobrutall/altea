...
export class SchemaSettings {
    ...
    // COMPATIBILITY with a database generated by Signum: name and shape things the way Signum does,
    // wherever altea's own convention would otherwise diverge for no reason but taste. A `terminal sync`
    // against such a database then reads as a MIGRATION (the model differences) instead of a rebuild
    // (every table renamed), which is what makes migrating an application — or running both against one
    // database — practical.
    //
