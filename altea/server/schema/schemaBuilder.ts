...
//
// LEGACY MODE joins with a SINGLE underscore instead (award_nomination_points), which is what Signum
...
// LEGACY MODE, the second rule (see SchemaSettings.legacyMode). Signum names an MList table after the
// OWNER's table plus the COLLECTION PROPERTY — `GenerateTableNameCollection` is literally
// `table.Name.Name + "_" + name` — where altea names a `@part` row table after the ROW ENTITY:
// `UserQueryEntity_Column` → `user_query_column` against Signum's `user_query_columns`. One table, two
// spellings, and the difference is not pluralization (`territories` is not `territory` + "s"): Signum's
// half of the name is the COLLECTION's own name, which nothing in the row type records.
//
// So resolve it the way Signum composes it — walk to the owner through the row's `@backReference`, find
// the ARRAY field on the owner that holds this row type, and hang it off the OWNER's table name. That
// last step goes through `settings.tableName`, so it recurses (a part row may itself own a part
// collection: dashboard → parts → …, Signum's `token_equivalence_group_token_equivalences`) and an app
// that overrode `tableName` still has its override respected, exactly as Signum reads `table.Name.Name`.
//
...
function legacyCollectionTableName(type: Type<Entity>, settings: SchemaSettings): string | undefined {
    ...
    const ownerInfo = getTypeInfo(owner);
    if (ownerInfo == null)
        return undefined;
...
export class SchemaSettings {
    ...
    // COMPATIBILITY with a database generated by Signum: name things the way Signum names them, wherever
    // altea's own convention would otherwise diverge for no reason but taste. A `terminal sync` against
    // such a database then reads as a MIGRATION (the model differences) instead of a rebuild (every table
    // renamed), which is what makes migrating an application — or running both against one database —
    // practical.
    //
    ...
    //      writes `role_inherits_from`) becomes a single one — inert on SQL Server, which produces none;
    //   2. a COLLECTION row is named after the owner's collection PROPERTY rather than the row entity
    //      (`user_query_columns`, not `user_query_column`) — see legacyCollectionTableName, which is where
    //      rule 1 stops being enough.
    // Expect more as the migration path is exercised — each one belongs here, gated on this same flag,
    // never as a second flag.
    legacyMode = false;
