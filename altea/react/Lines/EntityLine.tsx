...
import { type Aprox, EntityBaseController, type EntityBaseProps } from './EntityBase'
...
export interface EntityLineProps<V extends BaseEntity | Lite<Entity> | null> extends EntityBaseProps<V> {
  avoidLink?: boolean;
...
interface ItemPair {
  entity: BaseEntity | Lite<Entity>;
  item?: unknown;
}

export class EntityLineController<V extends BaseEntity | Lite<Entity> | null> extends EntityBaseController<EntityLineProps<V>,V> {
    ...
    React.useEffect(() => {
      ...
      if (p.autocomplete) {
        ...
        if (entity == null) {
          ...
          if (!this.currentItem || !(this.currentItem.entity as Entity | Lite<Entity>).is(entity as Entity | Lite<Entity>) || this.currentItem.entity.toString() != (entity as BaseEntity | Lite<Entity>).toString()) {
            var ci = { entity: entity!, item: undefined as unknown }
            this.setCurrentItem(ci);

            var fillItem = (newEntity: BaseEntity | Lite<Entity>) => {
  ...
  override overrideProps(p: EntityLineProps<V>, overridenProps: EntityLineProps<V>): void {
    ...
    if (p.type) {
      if (p.showType == undefined)
        p.showType = p.type.typeName.contains(",");
...
export const EntityLine: <V extends BaseEntity | Lite<Entity> | null>(props: EntityLineProps<V>) => React.ReactNode | null
  = genericMemo(function EntityLine<V extends BaseEntity | Lite<Entity> | null>(props: EntityLineProps<V>): React.ReactElement | null {
      ...
      if (p.view && !p.avoidLink) {
        return (
          <LinkButton ref={e => setLinkOrSpan(e)}
            ...
            title={p.ctx.titleLabels ? (value as BaseEntity | Lite<Entity>).toString() : undefined} {...p.itemHtmlAttributes}>
            {str}
          </LinkButton>
        );
      } else {
        return (
          <span tabIndex={0} ref={e => setLinkOrSpan(e)} title={(value as BaseEntity | Lite<Entity>).toString()} className={classes(p.ctx.formControlClass, "sf-entity-line-entity")} {...p.itemHtmlAttributes}>
            {str}
          </span>
        );
      }
