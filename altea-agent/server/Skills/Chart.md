...
* `AllTypes` you can provide any column.
...
The `chartScript` is the key of the ChartScript you want to use. 
...
* `orderByIndex` should be a number >= 0 that indicates the priority of the order (0 for first, 1 for second, etc).

## Converting a ChartOptions to a url

Use the tool `GetChartUrl`. It will validate the `ChartOptions` and return either an error message or the url, which you can show to the user as a markdown link.
