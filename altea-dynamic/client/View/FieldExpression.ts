...
// The one thing this package needs out of Signum's `TypeHelpComponent`: `getExpression`, which turns a
// stored property PATH into the JavaScript expression that reads it. TypeHelp itself is not ported (the
// honest equivalent is editor IntelliSense over the same `.d.ts`), so the function is re-homed here under a
// name that says what it does. Two callers: `NodeUtils.asFieldFunction` (the runtime accessor a node's
// `field` resolves to) and `CodeContext.subCtxCode` (the source the "Show code" modal prints).
//
// altea divergences:
...
//  - a MIXIN step (`[SomeMixin]`) becomes `.mixin(SomeMixin)` — a typed cast, where
...
//    members are camelCase. Members are camelCase in BOTH tiers here, so a stored path is already correct
