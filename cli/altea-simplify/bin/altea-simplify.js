...
(function(Console2) {
  ...
  async function askString(question) {
    ...
    handleSigInt(rl);
    try {
      return await new Promise((resolve2) => rl.question(question, resolve2));
    } finally {
      rl.close();
    }
  }
...
(function(Prompt2) {
  ...
  async function askValidated(question_, validate2, initial) {
    for (; ; ) {
      const answer = initial ?? await Console.askString(question_);
      initial = void 0;
      if (answer === "")
        return void 0;
      const complaint = validate2(answer);
      if (complaint == void 0)
        return answer;
  ...
  function question(rl, text) {
    return new Promise((resolve2) => rl.question(text, resolve2));
  }
})(Prompt || (Prompt = {}));
...
// altea-simplify/dist/Simplify.js
import * as fs3 from "node:fs";
import * as path3 from "node:path";

// altea-simplify/dist/ModulesXml.js
...
// altea-simplify/dist/ModulesXml.js
var ModulesXml;
(function(ModulesXml2) {
  function read(filePath, rootFolder, applicationName) {
    const xml = fs2.readFileSync(filePath, "utf8");
    ...
    const parser = new XMLParser({
      ...
      isArray: (name, _jpath, _leaf, isAttribute) => !isAttribute && !["File", "Modules", "Projects", "Workspaces"].includes(name)
    });
  ...
  ModulesXml2.read = read;
  function locate(rootFolder, applicationName) {
    return [path2.join(rootFolder, "Modules.xml"), path2.join(rootFolder, applicationName, "Modules.xml")].find((candidate) => fs2.existsSync(candidate));
  }
  ModulesXml2.locate = locate;
  function closeRemoval(modules, removing) {
  ...
  ModulesXml2.closeRemoval = closeRemoval;
})(ModulesXml || (ModulesXml = {}));
function readModule(raw) {
  const name = raw["@Name"];
  if (name == void 0)
...
function readDirective(module, kind, d) {
  const dependsOn = d["@DependsOn"];
  const need = (attribute) => {
    const v = d["@" + attribute];
    if (v == void 0)
  ...
  switch (kind) {
    ...
    case "RemoveLine": {
      ...
      if (line == void 0 && contains == void 0 && (from == void 0 || to == void 0))
...
(function(Simplify2) {
  async function run(uctx, options = {}) {
    ...
    const removing = options.keep != void 0 || options.remove != void 0 ? fromArguments(file, options) : await askInteractively(file, options.yes === true);
    if (removing == void 0) {
    ...
    if (options.dryRun !== true) {
      if (!Git.isRepository(uctx.rootFolder))
        throw new Error(`${uctx.rootFolder} is not a git repository. Simplify edits and deletes source in place; git is what makes that reviewable.`);
    ...
    Console.writeLine();
    Console.writeLineColor(options.dryRun === true ? Color.yellow : Color.green, options.dryRun === true ? "Dry run \u2014 nothing was changed." : "Done. Run `pnpm install` and build before committing anything else.");
  }
  Simplify2.run = run;
  async function askInteractively(file, yes) {
    const choices = file.modules.map((m) => ({
      key: m.name,
    ...
    Console.writeLine();
    Console.writeLineColor(Color.darkGray, "  Tick a module to KEEP it. Unticked modules are REMOVED from the source, one commit each.");
    Console.writeLineColor(Color.darkGray, "  Unticking a module unticks everything that depends on it; ticking one ticks what it needs.");
    const kept = await Prompt.multiSelect("Modules to keep", choices);
    if (kept == void 0)
      return void 0;
    const keptNames = new Set(kept.map((m) => m.name));
    const removing = ModulesXml.closeRemoval(file.modules, new Set(file.modules.filter((m) => !keptNames.has(m.name)).map((m) => m.name)));
    const cascaded = [...removing].filter((n) => keptNames.has(n));
    if (cascaded.length > 0) {
      Console.writeLine();
      Console.writeLineColor(Color.yellow, `  Also removing (they depend on something you removed): ${cascaded.join(", ")}`);
      if (!yes && !await Console.ask("  Continue?"))
        return void 0;
    }
    return removing;
  }
  function fromArguments(file, options) {
    const names = new Set(file.modules.map((m) => m.name));
    const check = (list, what) => {
      for (const n of list)
        if (!names.has(n))
          throw new Error(`--${what} names '${n}', which is not a module in Modules.xml. Valid: ${[...names].join(", ")}`);
      return list;
    };
  ...
  function applyModule(file, module, removing, dryRun) {
    ...
    for (const d of [...module.directives, ...meeting]) {
      try {
        ...
        Console.writeLineColor(Color.red, `   ${e.message}`);
      }
    }
  }
  function applyDirective(file, d, dryRun) {
    switch (d.kind) {
      case "RemoveFiles":
        return removeFiles(file, d.path, dryRun);
      case "RemoveLine":
        return removeLine(file, d, dryRun);
      case "RemoveSpanInLines":
        return replaceSpan(file, d.path, d.span, "", dryRun);
      case "ReplaceSpanInLines":
        return replaceSpan(file, d.path, d.span, d.with, dryRun);
      case "RemovePackageReference":
        return removePackageReference(file, d.name, dryRun);
      case "RemoveTsProjectReference":
        return removeTsProjectReference(file, d.path, d.reference, dryRun);
    }
  }
  function removeFiles(file, relative, dryRun) {
    const full = path3.join(file.rootFolder, relative);
    if (!fs3.existsSync(full)) {
      Console.writeLineColor(Color.darkGray, `   (already gone) ${relative}`);
      return;
    }
    const isDirectory = fs3.statSync(full).isDirectory();
    Console.writeLineColor(Color.yellow, `   delete ${isDirectory ? "directory " : ""}${relative}`);
    if (!dryRun)
      fs3.rmSync(full, { recursive: true, force: true });
  }
  function removeLine(file, d, dryRun) {
    edit(file, d.path, dryRun, (lines) => {
      if (d.line != void 0 || d.contains != void 0) {
        const matches = d.line != void 0 ? (l) => l.trim() === d.line.trim() : (l) => l.includes(d.contains);
        ...
        if (before === lines.length)
          return { changed: false, note: `no line ${d.line != void 0 ? "equals" : "contains"} ${what}` };
  ...
  function repairTrailingComma(relative, lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (next != void 0 && (next.trim().startsWith("}") || next.trim().startsWith("]")))
        lines[i] = lines[i].trimEnd().replace(/,$/, "");
    }
  }
  function replaceSpan(file, relative, span, replacement, dryRun) {
    edit(file, relative, dryRun, (lines) => {
      let hits = 0;
      for (let i = 0; i < lines.length; i++)
        while (lines[i].includes(span)) {
          lines[i] = lines[i].replace(span, replacement);
          hits++;
        }
      const what = replacement === "" ? `removed ${hits} occurrence(s) of ${JSON.stringify(span)}` : `replaced ${hits} occurrence(s) of ${JSON.stringify(span)} with ${JSON.stringify(replacement)}`;
      return hits === 0 ? { changed: false, note: `no line contains ${JSON.stringify(span)}` } : { changed: true, note: what };
    });
  }
  function removePackageReference(file, name, dryRun) {
  ...
  function tsconfigsOf(file, application) {
    const dir = path3.join(file.rootFolder, application);
    if (!fs3.existsSync(dir))
      return [];
    return fs3.readdirSync(dir).filter((n) => /^tsconfig.*\.json$/.test(n)).map((n) => `${application}/${n}`);
  }
  function removeTsProjectReferencesTo(file, relative, folder, dryRun) {
    const full = path3.join(file.rootFolder, relative);
    if (!fs3.existsSync(full))
      return;
    const marker = new RegExp(`["/]${escapeRegex(folder)}/tsconfig[^"]*\\.json"`);
    if (!marker.test(fs3.readFileSync(full, "utf8")))
      return;
    edit(file, relative, dryRun, (lines) => {
      let removed = 0;
      for (let i = lines.length - 1; i >= 0; i--) {
      ...
      return removed === 0 ? { changed: false, note: `no project reference to ${folder}` } : { changed: true, note: `removed ${removed} project reference(s) to ${folder}` };
    });
  }
  function removeTsProjectReference(file, relative, reference, dryRun) {
    edit(file, relative, dryRun, (lines) => {
  ...
  function edit(file, relative, dryRun, transform) {
    const full = path3.join(file.rootFolder, relative);
    if (!fs3.existsSync(full)) {
      Console.writeLineColor(Color.darkGray, `   (already gone) ${relative}`);
      return;
    ...
    if (changed && !dryRun)
      fs3.writeFileSync(full, lines.join(newline), "utf8");
  }
})(Simplify || (Simplify = {}));
  ...
  function run(file) {
    ...
    const readLines = (relative) => {
      if (!cache.has(relative)) {
        const full = path4.join(file.rootFolder, relative);
        cache.set(relative, fs4.existsSync(full) && fs4.statSync(full).isFile() ? fs4.readFileSync(full, "utf8").split(/\r?\n/) : void 0);
      }
...
${file.filePath}: ${problems.length} problem(s) over ${file.modules.length} modules.`);
    ...
    Console.writeLineColor(Color.green, `${file.filePath}: OK \u2014 ${file.modules.length} modules, every anchor resolves.`);
    return true;
  }
  Check2.run = run;
  function checkDirective(module, d, rootFolder, names, readLines, report) {
    if (d.dependsOn != void 0 && !names.has(d.dependsOn))
      report(module, `<${d.kind}> DependsOn names no such module: ${d.dependsOn}`);
    if (d.kind === "RemovePackageReference")
      return;
    if (d.kind === "RemoveFiles") {
      if (!fs4.existsSync(path4.join(rootFolder, d.path)))
    ...
    if (d.line != void 0) {
      if (!lines.some((l) => l.includes(d.line)))
        ...
        report(module, `${d.path}: Line= matches only part of a line (RemoveLine is whole-line): ${JSON.stringify(d.line)}`);
      return;
    }
    const from = d.from, to = d.to;
    for (const [attribute, needle] of [["From", from], ["To", to]])
    ...
    if (matches > 1)
      report(module, `${d.path}: From= matches ${matches} lines \u2014 the anchor is ambiguous: ` + JSON.stringify(from));
    ...
    if (end < 0)
      ...
      report(module, `${d.path}: span From=${JSON.stringify(from)} is ${end - start + 1} lines \u2014 the anchor is probably too generic`);
  }
})(Check || (Check = {}));

// altea-simplify/dist/main.js
...
try {
  if (args.flags.has("help") || args.flags.has("h")) {
    ...
    await Simplify.run(uctx, {
      keep: args.values.has("keep") ? splitList2(args.values.get("keep")) : void 0,
      remove: args.values.has("remove") ? splitList2(args.values.get("remove")) : void 0,
