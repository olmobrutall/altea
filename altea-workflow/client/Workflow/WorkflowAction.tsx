...
import { WorkflowActionEval, type WorkflowActionEntity } from "../../data/WorkflowAction";
import { WorkflowClient } from "../WorkflowClient";

// Port of Signum.Workflow's Workflow/WorkflowAction.tsx — the name, the main entity type, and the SCRIPT.
//
// altea divergences:
//  - the editor is TypeScript rather than C# (@altea/altea-eval's EvalLine, which draws Signum's
//    signature / editor / closing-brace sandwich; the C# one read `void Execute(OrderEntity e, WorkflowTransitionContext ctx)`).
...
export default function WorkflowAction(p: { ctx: TypeContext<WorkflowActionEntity> }): React.JSX.Element {
    ...
    function handleMainEntityTypeChange(): void {
        ...
        ctx.value.eval ??= WorkflowActionEval.create({ script: "" });
        ...
        <div>
            ...
            {cleanName != null && ctx.value.eval != null &&
                <EvalLine ctx={ctx.subCtx(d => d.eval)}
                    signature={`function evaluate(e: ${cleanName}, ctx: WorkflowTransitionContext): Promise<void>`} />}
        </div>
    );
}
