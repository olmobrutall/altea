...
// entities, where Signum's script calls a generated `CreateCase(entity)` helper.
//
// The `@column(false) fullWorkflow` cache is Signum's, but the RESOLUTION lives in WorkflowEventTaskLogic
// (`getWorkflow(task)`) rather than on the entity — a data-layer class holding a mutable hook into the
// server is exactly the coupling the layer split exists to prevent. `getModel` / `applyModel` are on the
// server for the same reason: they read and write ScheduledTaskEntity rows.
...
export class WorkflowEventTaskEntity extends Entity implements ITaskEntity {
    ...
    /** Set when the task is built in memory
...
/**
 * "should this scheduled start fire now?". It takes NOTHING: there
 ...
 *
 * The action RETURNS the entities, where Signum's generated wrapper gives the script a `CreateCase(entity)` method that pushes
