# altea — the Signum port ledger

**What changed on the way from Signum Framework to altea, and why.** Every entry is a deliberate
divergence: the rule, the reason, and — where an existing database is affected — the migration it
needs. Written while porting; kept because it is what makes a future Signum change re-appliable, and
what tells a reader of Signum's source what the altea counterpart is called.

The CONVENTIONS distilled out of this — what a developer writing altea code has to follow — live in
[`../AGENTS.md`](../AGENTS.md). This file is the reasoning behind them. Per-module ledgers are the
sibling files in this directory (see [README.md](README.md)).

The source being ported from is Signum + Southwind, kept read-only as a sibling checkout:

| From (C# / TypeScript) | To (TypeScript) | What it is |
| --- | --- | --- |
| **C#** | **TypeScript** | language |
| **Signum Framework** — `Framework/` (C# engine) + `Framework/Signum/React/` (React/TS client) | **altea** — `altea/altea/` | the framework (ORM, LINQ provider, dynamic queries, reflection, React UI kit) |
| **Southwind** (Signum's demo app) | **eastwind** | the demo application built on the framework |

> Signum's own `CLAUDE.md` / `AGENTS.md` describe **Signum's** conventions, not altea's — read them to
> understand the source, but altea's conventions win.
...
- **`XxxInfo` vs `XxxMetadata` — the two halves of reflection.** `TypeInfo` / `FieldInfo` (`data/reflection`) are the **compile-time** descriptor the quote-transformer stamps onto each constructor: types, units, formats, validators, implementations — identical for every user and every culture. `TypeMetadata` / `FieldMetadata` / `OperationMetadata` (`data/metadata`) are the **runtime** half: per-CULTURE (nice names, plural, gender) and per-ROLE (`maxTypeAllowed`, `routes[path].propertyAllowed`, and whether the type is in the blob AT ALL — presence is the read permission), assembled per request by `ReflectionServer.buildMetadata`, squeezed into the compact `MetadataBlobWire` by `toWire` and expanded back by `Metadata.fromWire` on arrival, so only those two functions know the encoding. A `TypeMetadata` keeps the two apart: **`fields`** is the type's OWN members by bare name — what `FieldInfo.niceToString()` asks for with `(declaringType, member)`, so an embedded, a `@part` and a mixin each describe their own members once — while **`routes`** is owner-rooted property routes, which only a persisted Entity has and only a rule writes. `Metadata.tryField` / `Metadata.tryRoute` are the two lookups. Structurally this follows Signum — one entry per type carrying everything about it — but Signum's single `TypeInfo` family is split, so nothing per-role ever lands on the compile-time descriptor. Consequences: `TypeInfo` has NO `operations` / `hasConstructorOperation` / `gender`; read them via `Metadata.tryType(name)` or the client's `getOperationInfos` / `ti.getGender()`.
...
- **Directory login: ONE authorizer, ONE shared base, and no server-rendered config blob.** Signum copies
  ...
  **[altea/port/AuthDirectory.md](altea/port/AuthDirectory.md)**.
...
- **A REMOTE file store cannot rename, and cannot be read synchronously.** `altea-files` splits a save into
  ...
  **[altea/port/FileStores.md](altea/port/FileStores.md)**.
...
- **The mail SERVICES are a registry, and each protocol package fills one slot.** `EmailServiceEntity`
  ...
  **[MailingExchange.md](altea/port/MailingExchange.md)**,
  **[MailingPop3.md](altea/port/MailingPop3.md)**.
...
- **The diff log is TWO core seams plus a mixin.** altea-diff-log stores the before/after dumps of an
  ...
  the columns don't. Full ledger: **[altea/port/DiffLog.md](altea/port/DiffLog.md)**.
...
- **Signum.Dynamic ports WHOLE, and the compiled half rests on running the quote-transformer at RUNTIME.**
  ...
  Full ledger: **[altea/port/Dynamic.md](altea/port/Dynamic.md)**.
...
- **SignalR → a WebSocket hub in altea CORE.** Node has no SignalR server and `@microsoft/signalr` speaks a
  ...
  else. That module's ledger: **[altea/port/ConcurrentUser.md](altea/port/ConcurrentUser.md)**.
...
- **Signum.Workflow → altea-workflow: `withQuoted` is query-only.** The BPMN engine ports whole (designer,
  ...
  Full ledger: **[altea/port/Workflow.md](altea/port/Workflow.md)**.
...
- **`TypeEntity` keeps Signum's `namespace` beside altea's `package`, and the sync CARRIES IT OVER.**
  ...
  - **`isPart` is a STORED column, and altea is what FILLS it** — Signum declares it (`public bool?`, a
    column it neither fills nor copies on a merge, exactly as it treats `package`) and has no such rows
    to hide of its own. altea models a Signum MList element and several owned embeddeds as `@part`
    ...
    - **a column and not the client-side `TypeInfo.entityKind`, which already ships.** A client-side
      ...
      whole justification for a column at all — which is why Signum declares it NULLABLE and altea
      non-nullable: altea derives it for every row it writes, Signum leaves it to whoever knows.
    ...
    - **KEPT in legacy mode — deliberately NOT hidden through `simplifyDiffTables`.** Hiding it there
      ...
      already makes, and one a Signum deployment then leaves alone.
...
- **`@legacyColumnName` — the field-level sibling of `@legacyTableName`, and the seam that keeps a model
  ...
  ledger: **[altea/port/OfficeTemplate.md](altea/port/OfficeTemplate.md)**.
...
- **Token migrations: the renames a schema sync resolves are replayed against the query TOKENS stored
  ...
  for column. Full ledger: **[altea/port/UserAssets.md](altea/port/UserAssets.md)**.
...
- **Signum.Rest → altea-rest: an MVC action filter becomes EXPRESS MIDDLEWARE** mounted on a path prefix
  ...
  **[altea/port/Rest.md](altea/port/Rest.md)**.
...
- **Signum.ViewLog → altea-view-log: the module IS three subscriptions**, and it forced three core seams —
  ...
  **[altea/port/ViewLog.md](altea/port/ViewLog.md)**.
...
- **Signum.Markdown → altea-markdown: Markdig becomes mdast**, which is the parser react-markdown already
  ...
  **[altea/port/Markdown.md](altea/port/Markdown.md)**.
...
- **Signum.Printing → altea-printing: a queue whose last step is an app seam.** A document producer drops a
  ...
  Finished. Full ledger: **[altea/port/Printing.md](altea/port/Printing.md)**.
...
- **Signum.Excel's ExcelReport half → altea-office-template: a stored WORKBOOK, refilled.** The package's
  ...
  **[altea/port/OfficeTemplate.md](altea/port/OfficeTemplate.md)**.
...
- **Signum.MachineLearning → altea-machine-learning: a CODIFICATION is the unit, and CNTK becomes
  ...
  Full ledger: **[altea/port/MachineLearning.md](altea/port/MachineLearning.md)**.
