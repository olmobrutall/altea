...
- **`#DatabaseName#` is not ported.** Signum replaced it so a script could name its own database
  (cross-database references); altea's schema has no database-qualified names to rewrite and its Connector
  exposes no database name, so a script carrying the token cannot be executed faithfully — `execute` says
