...
//
// The input is a native `datetime-local`, whose value IS the ISO shape `Temporal.PlainDateTime` parses — in
// the viewer's zone, so the value crosses Clock.toUserInterface / fromUserInterface on the way in and out.
...
function toInputValue(value: Temporal.PlainDateTime): string {
    return Clock.toUserInterface(value).toString({ smallestUnit: "minute" });
}

function parse(value: string): Temporal.PlainDateTime | undefined {
    try {
        return value === "" ? undefined : Clock.fromUserInterface(Temporal.PlainDateTime.from(value));
    } catch {
        return undefined;
    }
}
