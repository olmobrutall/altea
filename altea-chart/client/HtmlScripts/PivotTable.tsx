...
import { ChartClient, type ChartScriptProps, type ChartColumn, type ChartRow } from '../ChartClient';
...
import type { FilterOptionParsed, FilterConditionOptionParsed } from '@altea/altea/client/FindOptions';
...
// altea Array/String prototype extensions (.sum/.groupBy/.toObject/.distinctBy/.etc/…).
import '@altea/altea/data/globals/arrayExtensions';
import '@altea/altea/data/globals/stringExtensions';

// Copy-and-fix of Signum.Chart/HtmlScripts/PivotTable.tsx — the first Html renderer (returns an HTML table,
// not an <svg>). altea fixes vs Signum:
//  - @framework/* → @altea/altea/*; Color from the altea-chart-local ../Color (lerp added for cell text).
//  - `token.queryTokenType == "Aggregate"` → `token instanceof AggregateToken`.
//  - `token.type.name == "boolean"` → `token.filterType == "Boolean"`.
//  - Signum's `BooleanEnum.niceToString("True"/"False")` → "True"/"False" literals (altea has no BooleanEnum
//    localized container; enum wire values are member-name strings anyway).
//  - `QueryToken.fullKey`/`niceName` are METHODS in altea (`.fullKey()`, `.niceName()`).
//  - Signum's `QueryTokenMessage.Null` (the "-Null-" group label) has no altea equivalent → literal "-Null-".
//  - `.contains` → `.includes`; dropped the `as ChartParameter` casts (altea `parameters` is a plain
//    string-indexed Record; there is no ChartParameter string-union type).
//  - `isLite(x)` → `x instanceof Lite`.
...
class RowGroup {
  ...
  getNiceName() {
    if (this.column.token?.filterType == "Boolean")
...
function getMultiAggregator(columns: ChartColumn<number>[]): (values: MultiNum[]) => MultiNum {
  ...
  function getSingleAggregator(token: QueryToken): (values: MultiNum[], selector?: (mn: number[]) => number, countSelector?: (mn: number[]) => number) => number {
    if (!(token instanceof AggregateToken))
  ...
  function getCountNNSelectors(c: ChartColumn<number>) {
    if (c.token instanceof AggregateToken && c.token.key == "Average") {
      const countNNColumn = columns.firstOrNull(s => s.token instanceof AggregateToken && s.token.key == "CountNotNull" && s.token.parent?.fullKey() == (c.token as AggregateToken).parent?.fullKey());

      if (countNNColumn == null)
        throw new Error(`Unable to combine values an Avg token (${c.token.fullKey()}) because no equivalent CountNotNull token found (${(c.token as AggregateToken).parent?.fullKey() + ".CountNotNull"})`);
...
export default function renderPivotTable({ data, width, height, parameters, loading, onDrillDown, initialLoad, chartRequest, onReload, dashboardFilter }: ChartScriptProps): React.ReactElement<any> {
  ...
  // No role on the empty state: its only content is a message, which role="img" would flatten away
  // behind a picture that has no name either.
  if (data == null)
    return (
  ...
  function Cell(p:
    ...
    function handleNumberClick(e: React.MouseEvent<HTMLAnchorElement>) {
      ...
      onDrillDown(filters.toObject(a => a.col.name, a => a.val) as ChartRow, e);
    }

    var lite = gr && gr.value instanceof Lite ? gr.value : undefined;
    ...
    var titleElement = lite instanceof Lite ?
