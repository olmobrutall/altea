...
// altea's Array/String prototype extensions (Signum ships these as globals too). The side-effect import
// installs the methods at runtime AND brings the `declare global` augmentation into the program, so
// `.toObject` / `.after` type-check here (and program-wide for the chart client).
import "@altea/altea/data/globals/arrayExtensions";
import "@altea/altea/data/globals/stringExtensions";

// Copy-and-fix of Signum.Chart/ColorPalette/ColorUtils.ts. Verbatim except imports: d3 / d3-scale-chromatic
// resolve as real deps, and `.toObject` / `.after` are altea's Array/String prototype extensions (same API
// as Signum's), so the body is unchanged.
...
export const colorInterpolators: { [name: string]: Interpolator } = [
  ...
  { categoryName: "Multi-Hue", name: "YlGn", interpolate: d3sc.interpolateTurbo }, // matches Signum (likely a typo for interpolateYlGn)
