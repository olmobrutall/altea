...
import type { PivotColumn, PivotTable } from './PivotTable';

// Copy-and-fix of Signum.Chart/D3Scripts/Components/Legend.tsx (the multi-series color legend). Verbatim
// except imports (PivotColumn/PivotTable via import type).
    ...
    <g>
      <g className="color-legend" transform={translate(xRule.start('content'), yRule.start('legend'))}>
        {pivot.columns.map((s, i) =>
          <g key={s.key} style={p.onDrillDown ? { cursor: "pointer" } : undefined} onClick={e => p.onDrillDown?.(s, e)} opacity={p.isActive && p.isActive(s) == false ? .5 : undefined}>
