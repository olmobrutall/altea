...
import { scaleFor } from "./Components/ChartUtils";
...
// scale on the right-hand side. Fixes: @framework/* → altea paths, `../Signum.Chart` → `../../data/ChartScript`
// (D3ChartScript symbols), and the unused translate/scale/rotate/… ChartUtils re-exports dropped.
...
export function renderCombinedLinesAndColumns({ infos, width, height, initialLoad, useSameScale }: {
    infos: ReactChartCombinedInfo[], width: number, height: number, initialLoad: boolean, useSameScale: boolean,
}): React.ReactElement {
    ...
    const notSupported = infos.filter(info => !supportedTypes.includes(info.chartRequest.chartScript.key));
    if (notSupported.length > 0)
        throw new Error(`Combined Chart only supports ${supportedTypes.joinComma(" and ")}, not ${notSupported.map(a => a.chartRequest.chartScript.key).joinComma(" or ")}`);
    ...
    if (otherKeyColumn.length)
        throw new Error(`Incompatible key columns for the horizontal axis: ${otherKeyColumn.map(a => a.data!.columns.c0!.type).joinComma(" and ")} instead of ${keyColumn.type}`);
    ...
    const keyValues = ChartUtils.completeValues(keyColumn, allKeyValues, firstParameters["CompleteValues"],
        infos[0].chartRequest.filterOptions, ChartUtils.insertPoint(keyColumn, valueColumn));
    ...
    if (useSameScale) {
        ...
        const scale = scaleFor(valueColumn, allValues, 0, yRule.size("content"),
            firstParameters["VerticalScale"] ?? firstParameters["Scale"]);
        ...
        yScales = infos.map(info => {
            ...
            return scaleFor(valColumn, info.data.rows.map(r => valColumn.getValue(r)), 0, yRule.size("content"),
                info.parameters["VerticalScale"] ?? info.parameters["Scale"]);
        });
    }
    ...
    return (
        <svg direction="ltr" width={width} height={height} role="img">
            ...
            {!useSameScale && yScales[1] &&
                <YScaleTicksEnd xRule={xRule} yRule={yRule} valueColumn={infos[1].data!.columns.c1 as ChartColumn<number>} y={yScales[1]!} />}

            {
                infos.map((info, i) => info.data == null ? null :
                    <g key={i}>
                        {
                            info.chartRequest.chartScript.key == D3ChartScript.Line.key
                                ? paintLine({
                                    xRule, yRule, keyValues, data: info.data, initialLoad, onDrillDown: info.onDrillDown,
                                    parameters: info.parameters, hasHorizontalScale: false, x: x, y: yScales[i]!, memo: info.memo,
                                })
                                : info.chartRequest.chartScript.key == D3ChartScript.Columns.key
                                    ? paintColumns({
                                        xRule, yRule, keyValues, data: info.data, initialLoad, onDrillDown: info.onDrillDown,
                                        parameters: info.parameters, hasHorizontalScale: false, x: x, y: yScales[i]!,
                                        colCount, colIndex: colIndex++, memo: info.memo,
                                    })
                                    : null
                        }
                    </g>
            ...
            <InitialMessage data={infos.firstOrNull(a => a.data != null)?.data} x={xRule.middle("content")} y={yRule.middle("content")}
                loading={infos.some(a => a.data == null)} />
            <XAxis xRule={xRule} yRule={yRule} />
            <YAxis xRule={xRule} yRule={yRule} />
