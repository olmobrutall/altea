...
import type { ChartScriptProps, ChartColumn } from '../ChartClient';
import * as ChartUtils from './Components/ChartUtils';
import { translate, scale, rotate, scaleFor } from './Components/ChartUtils';
import { toPivotTable, groupedPivotTable } from './Components/PivotTable';
...
// Copy-and-fix of Signum.Chart/D3Scripts/MultiColumns.tsx. Standard fixes; the c.c1 && … Legend props
...
export default function renderMultiColumns({ data, width, height, parameters, loading, onDrillDown, initialLoad, chartRequest, memo, dashboardFilter }: ChartScriptProps): React.ReactElement<any> {
  ...
  return (
    <svg direction="ltr" width={width} height={height} role="img">
      <title id="multiColumnsChartTitle">{ChartMessage._0Of1_2Per3.niceToString(ChartClient.symbolNiceName(D3ChartScript.MultiColumns), getQueryNiceName(chartRequest.queryKey), [keyColumn.title, valueColumn0.title].join(", "), [c.c1, c.c3, c.c4, c.c5, c.c6].filter(cn => cn != undefined).map(cn => cn!.title).join(", "))}</title>
      ...
      <Legend pivot={pivot} xRule={xRule} yRule={yRule} color={color}
        isActive={c.c1 != null && detector != null ? (row => detector!({ c1: row.value })) : undefined}
        onDrillDown={c.c1 != null ? ((s, e) => onDrillDown({ c1: s.value }, e)) : undefined} />
