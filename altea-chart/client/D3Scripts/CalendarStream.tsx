...
import type { ChartColumn, ChartRow, ChartScriptProps, ChartTable } from '../ChartClient';
import * as ChartUtils from './Components/ChartUtils';
import { translate, rotate, scaleFor } from './Components/ChartUtils';
...
// day:-1})→new Date(y,m,0); Settings.defaultLocale→undefined so Intl uses the runtime default locale).
...
export function CalendarYear({ year, rules, rowByDate, width, height, onDrillDown, initialLoad, transform, monday, valueColumn, color, isHorizontal, dashboardFilter, chartRequest }: {
  ...
  function monthPosition(i: number) {
    // luxon DateTime.local(year, i, 1) (1-based month) → new Date(year, i-1, 1); the luxon
    // `.plus({month:1, day:-1})` (last day of month i) → new Date(year, i, 0).
    var firstDay = week(new Date(year, i - 1, 1));
    var lastDay = week(new Date(year, i, 0));
...
function getMonthsNames() {
  var format = new Intl.DateTimeFormat(undefined, { month: "short" });
  var months = []
...
function getWeekDayNames(monday: boolean) {
  var format = new Intl.DateTimeFormat(undefined, { weekday: "narrow" });
  var months = []
