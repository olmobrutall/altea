...
import { translate, scaleFor } from './Components/ChartUtils';
...
// Copy-and-fix of Signum.Chart/D3Scripts/ParallelCoordiantes.tsx (Signum misspelled the source file;
// the altea file/class are correctly spelled "ParallelCoordinates"). Standard fixes; the C# string
// `queryTokenType == "Aggregate"` checks become `token instanceof AggregateToken`, and the Signum-only
// `as ChartParameter` cast on the parameters index is dropped (altea's `parameters` is string-indexed).
...
function ParallelCoordinatesImp({ data, width, height, parameters, loading, onDrillDown, initialLoad, chartRequest, dashboardFilter }: ChartScriptProps) {
  ...
  var keyColumns: ChartColumn<any>[] = data.columns.entity ? [data.columns.entity] :
    [data.columns.c1, data.columns.c2, data.columns.c3, data.columns.c4, data.columns.c5, data.columns.c6, data.columns.c7, data.columns.c8].filter((cn): cn is ChartColumn<unknown> => cn != undefined).filter(a => a.token && !(a.token instanceof AggregateToken))

  var aggregateColumns: ChartColumn<any>[] = data.columns.entity ? [data.columns.entity] :
    [data.columns.c1, data.columns.c2, data.columns.c3, data.columns.c4, data.columns.c5, data.columns.c6, data.columns.c7, data.columns.c8].filter((cn): cn is ChartColumn<unknown> => cn != undefined).filter(a => a.token && a.token instanceof AggregateToken)

  var titleMessage = (aggregateColumns.length != 0) ?
    ChartMessage._0Of1_2Per3.niceToString(ChartClient.symbolNiceName(D3ChartScript.ParallelCoordinates), getQueryNiceName(chartRequest.queryKey), keyColumns.map(cn => cn.title).join(", "), aggregateColumns.map(cn => cn.title).join(", ")) :
